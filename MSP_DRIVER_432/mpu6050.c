...
int dr_MPU6050_atualizar(mpu_data_t *sensor)
{
	...
	{
		status_erro = dr_MPU6050_Read(sensor->I2C, sensor->address, 0x3A, &ready);
		if (status_erro != 1)
		{
...
int dr_MPU6050_init(mpu_data_t *sensor)
{
	...
	status_erro = dr_MPU6050_Read(sensor->I2C, sensor->address, 0x75, &data) - 10;
	if (data != 0x68) {
		return status_erro;
	}
...
int dr_MPU6050_Read(uint8_t n_I2C,
	uint8_t slaveAddr,
	uint8_t memAddr,
	uint8_t *data)
{
	...
	while (moduleI2C->CTLW0 & 2) ; /* wait till restart is finished */
	moduleI2C->CTLW0 |= 0x0004; /* setup to send STOP after the byte is received */
	while (!(moduleI2C->IFG & 1)) ; /* wait till data is received */ // flag de erro aqui ?
	*data = moduleI2C->RXBUF; /* read the received data */
	while (moduleI2C->CTLW0 & 4) ; /* wait until STOP is sent */
	// dr_I2C_read() end
		return 1;
}
int dr_MPU6050_ReadRaw(uint8_t n_I2C,
	uint8_t slaveAddr,
	uint8_t memAddr,
	uint8_t byteCount,
	uint8_t *data)
{
	...
	moduleI2C->CTLW0 |= 0x0010; /* enable transmitter */
	moduleI2C->CTLW0 |= 0x0002; /* generate START and send slave address */
	while ((moduleI2C->CTLW0 & 2)) ; /* wait until slave address is sent */
	moduleI2C->TXBUF = memAddr; /* send memory address to slave */
	while (!(moduleI2C->IFG & 2)) ; /* wait till last transmit is done */
	moduleI2C->CTLW0 &= ~0x0010; /* enable receiver */
	moduleI2C->CTLW0 |= 0x0002; /* generate RESTART and send slave address */
	while (moduleI2C->CTLW0 & 2) ; /* wait till RESTART is finished */
	...
	do
	{
		...
		while (!(moduleI2C->IFG & 1)) ; /* wait till data is received */
		*data++ = moduleI2C->RXBUF; /* read the received data */
		byteCount--;
	} while (byteCount);
