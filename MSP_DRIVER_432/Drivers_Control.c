...
void dr_I2C_Read(uint8_t n_I2C, uint8_t slaveAddr, uint8_t memAddr, uint8_t *data)
{
	/* Selecionar I2C */
	...
	while (!(moduleI2C->IFG & 2)) ; /* wait till it's ready to transmit */
...
void dr_I2C_Write(uint8_t n_I2C, uint8_t slaveAddr, uint8_t memAddr, uint8_t data)
{
	/* Selecionar I2C */
	EUSCI_B_Type *moduleI2C;
	if (n_I2C == 0)
...
int dr_I2C_ReadRaw(uint8_t n_I2C,
	uint8_t slaveAddr,
	uint8_t memAddr,
	uint8_t byteCount,
	uint8_t *data)
